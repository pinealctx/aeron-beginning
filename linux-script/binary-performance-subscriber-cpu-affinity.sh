#!/bin/bash
# binary-performance-subscriber-cpu-affinity.sh
# 
# 🚀 CPU亲和性优化的UDP订阅者 - 绑定核心4-5

export AERON_DIR="/dev/shm/aeron"

echo "🚀 启动Aeron UDP订阅者 (CPU亲和性优化版)..."
echo "CPU绑定: 核心4-5 (专用)"
...
# CPU亲和性绑定到核心4-5，以原用户身份运行Java
if [ -n "$SUDO_USER" ]; then
    echo "🔧 以用户$SUDO_USER身份启动，绑定到CPU核心4-5..."
    taskset -c 4-5 nice -n -10 sudo -u "$SUDO_USER" "$JAVA_BIN" --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED \
    ...
    echo "🔧 以root身份启动，绑定到CPU核心4-5..."
    taskset -c 4-5 nice -n -10 "$JAVA_BIN" --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED \
        -Daeron.dir="$AERON_DIR" \
        -Xms2g -Xmx2g \
        -XX:+UseG1GC \
        -XX:MaxGCPauseMillis=2 \
